...
void find_and_save_words(Word words[], int num_words, char** most_frequent_long_word_ptr, char** least_frequent_short_word_ptr) {
    FILE* file = fopen("C:/Users/danik/Desktop/config.txt", "a");
    if (file == NULL) {
        printf("Ошибка при открытии файла config.txt\n");
        exit(1);
    }
...
void replace_words_in_file(char* filename, char* word_a, char* word_b) {
    ...
    FILE* new_file = fopen("C:/Users/danik/Desktop/compressed.txt", "w");
    if (new_file == NULL) {
        printf("Ошибка при создании файла new_text.txt\n");
        exit(1);
    }
    char buffer[MAX_WORD_LENGTH];
    char* line = NULL;
    ...
    fclose(file);
    fclose(new_file);
    int new_size = get_file_size("C:/Users/danik/Desktop/compressed.txt");
    int old_size = get_file_size(filename);
    if (new_size < old_size) {
        remove(filename);
        rename("C:/Users/danik/Desktop/compressed.txt", filename);
    }
    else {
        remove("C:/Users/danik/Desktop/compressed.txt");
    }
}
...
void append_config_to_file(char* filename) {
    ...
    FILE* config_file = fopen("C:/Users/danik/Desktop/config.txt", "r");
    if (config_file == NULL) {
        printf("Ошибка при открытии файла config.txt\n");
        exit(1);
    }
